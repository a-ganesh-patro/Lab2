...
class link //create a class of type link
{
  ...
  public:
    link () //constructor
    {
      head = NULL;  //make a head null
      tail = NULL;  //make the head null
    }
...
{ cout<<"\n This is the list of thing we can do using this programme."<<endl;
  ...
  cout<<" (e) delete data of the specific position."<<endl;
  cout<<" (f) display the elements of linked list."<<endl;
  cout<<" (g) count the number of elements present in the list."<<endl;
  cout<<" Press 'x' and hit 'enter' to exit."<<endl;
  cout<<" Choose the option what we want to do : \n -> ";
}
...
{ char ch;  //character type variable
 ...
 while (ch != 'x')  //condition : input is not x otherwise break the loop
   { menu();
     cin>> ch;
     //give different output depending on  character variable using 'if else'
     if (ch=='a') 
     {
     ...
     {
         int num;
         cout<<"\n How many data do we want to add ? \n ->";
         cin>>num;
         for(int i=1;i<=num;i++)
         {   int data;
             cout<<"Data "<<i<<" = ";
     ...
     {   int post, data;
         ...
         cout<<"\n Enter the position where we want to insert the data : ";
         cin>>post;
     ...
     else if (ch=='e')
     {
         ...
         int num;
         num = l_l.count_l();  //calling the count_l() function to check the number element in the list
         cout <<"\n Enter the position of data we want to delete : ";
         cin >> post;
     ...
     {   int num;
        ...
        {
             cout<<"\n we have entered an invalid key try again.";  //when one enter a invalid character
        }

     cout <<"\n  Thank you for using this programme.";
     cout <<"\n  If we want to continue press any key and hit 'enter'.";  //run the while loop again
     cout <<"\n  If we want to exit press 'x' and hit 'enter'. \n -> ";   //break the while loop
     cin >> ch; 
   }
...
void link :: insert_list(int data)
{
     ...
     temp->data = data;   //data input is inserted in the data part of temp
...
void link :: delete_list()
{
    ...
    { 
        ...
        tail=pos;    //make 'tail' and 'pos' become sames
        pos=pos->next;   //make the next node new 'pos'
        tail->next = NULL;
...
{ 
	node *pos;     // now u create a new node type pointer called 'pos' .
  pos=head;   // this function makes the address of 'pos' and 'head' same
  for (int i=2;i<post;i++)
  {
      pos=pos->next;
  }
  node *temp;  //create the new node type pointer called 'temp'
  temp = pos->next;
  pos->next = temp->next;
  temp->next = NULL;
  delete temp;  //delete the node 'temp'
}

void link :: display_l()
{
    node *pos;  //create the new node type pointer called 'pos'
    pos=head;  //makes the address of 'pos' and 'head' same
    if (head==NULL)
    {
    ...
    else
    {
       while (pos != NULL)
      {
         ...
         pos=pos->next; //goes to the next element 
      }
    }
}
...
int link :: count_l()
{
    ...
    pos = head;  //makes the address of 'pos' and 'head' same
    int i=2;
    if (head == NULL)   //when the number of element in the list is zero
    {
        return 0;
    }

    else if (pos->next == NULL) //when the number of element in the list we find is one
    {
        return 1;
    }
    ...
    else
    {
        ...
        return i; //return the number of elements present
    }
}
